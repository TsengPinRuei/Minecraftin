...
- 洞穴、海平面水體與樹木生成，搭配 Minecraft 式水源/流動水模擬：破壞方塊時水會逐步流入、擋住水源時失去支撐的水會退去、兩個水源可生成新水源。
- 第一人稱移動，包含走路、衝刺、蹲下（視角平滑過渡）、跳躍、游泳、爬梯子與創造模式飛行。
...
| 飛行時按 `Left Shift` | 向下飛 |
| 在水中按 `W` / `A` / `S` / `D` | 水平游泳 |
| 在水中按 `Space` | 向上游（放開會緩慢下沉） |
| 在水中按 `Left Shift` | 下潛 |
| 在水中雙擊 `Space` | 切換創造模式飛行 |
| 在梯子上按 `W` 或 `Space` | 往上爬 |
